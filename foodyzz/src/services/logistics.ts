...
export interface RentalQuote {
  ...
  // What the customer is charged at delivery. rent/buy: the whole committed term.
  // rent-to-buy: the first month only (the rest are billed monthly after delivery).
  total: number;
  // rent-to-buy only: recurring per-period subtotal (rate + recurring fees, pre-tax).
...
export const computeQuote = (
  ...
  // Rent-to-buy's term is fixed at the model's minimum commitment (billed monthly);
  // plain rent lets the customer pick the number of weeks.
  const minCommit = config.bikeModels.find((b) => b.model === model)?.minCommitment?.[
    rentalType as 'rent' | 'rentToBuy'
  ] ?? 0;
  const periods =
    rentalType === 'buy' ? 1
      : rentalType === 'rentToBuy' ? Math.max(1, minCommit || durationValue)
  ...
  // Fees are billed once per rental period — NOT per week/month of the term. So a
  // 4-week rent pays the base rate ×4 but the fee bundle once.
  const deposit = (config.fees || []).find((f) => f.isDeposit)?.amount ?? 0;
  // rent-to-buy charges ONE period now (rate + fees); the rest are billed monthly after
  // delivery, so `total` here is the first month, not the whole commitment.
  ...
  const total =
    rentalType === 'buy'
      ...
      : rentalType === 'rentToBuy'
        ? perPeriod
        : round2(baseRate * periods + recurringFees + oneTimeFees);
