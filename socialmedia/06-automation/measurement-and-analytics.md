...
**Rent-to-buy completion rate is the number that decides whether the business model works.** A customer is worth $887.68 over eight months only if they finish. Track it from the first cohort — the dunning path (retry at 24h, `past_due` after two failures) means the data will exist in `settlements/`.
